...
本次目标是 `codex/client-cli-completion-20260907` 分支的精确源码提交，程序版本仍为 `0.1.0-rc.3`，不是新的正式发行。`version --format json` 同时报告源码、目标平台、业务协议和本地格式身份。
...
| 全新 macOS → CLI 验证提交 | 新平台，使用 Unix 后端；上报真实 Intel/Apple Silicon 架构 | 必须先部署能识别新平台的 Server。分别通过原生 CI 后再确定发行支持 |
