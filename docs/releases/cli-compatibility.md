# Sunshine Client 0.2.5 兼容边界

当前正式版本为 `0.2.5`，支持 Windows x64、Linux x64 和 macOS Apple Silicon。部署时同时核对版本、源码
...
0.2.5 不把旧 Bootstrap、旧协议身份或旧任务当作当前输入，也不猜测式降级。Client 会把 v1 配对资料识别为
