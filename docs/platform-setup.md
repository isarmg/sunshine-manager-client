...
适用于 0.2.6。Client 是管理代理；需要先安装 Sunshine 并在其 Web UI 设置用户名、密码。Manager 地址、Manager 配对码、Sunshine 本机地址和 Sunshine 管理凭据是不同的输入。每个平台只发布一个原生安装包，安装器检查平台、架构、权限并注册服务。安装后的逐步配置和配对命令见[完整配置指南](configuration.md)。
...
```powershell
msiexec.exe /i .\sunshine-client-0.2.6-windows-x64.msi /norestart
...
MSI 同版文件修复：`msiexec.exe /i "完整路径\sunshine-client-0.2.6-windows-x64.msi" REINSTALL=ALL REINSTALLMODE=amus /l*v "%TEMP%\sunshine-client-install.log"`（在 cmd 中执行）。安装器忙碌时等待其他安装结束；3010 表示 Windows 需要重启完成替换。使用 `Get-Service SunshineClient` 和 `sunshine-client service status` 检查服务。

## Ubuntu 24.04 x86_64

```sh
sudo apt install ./sunshine-client_0.2.6_amd64.deb
...
同版损坏执行 `sudo apt install --reinstall ./sunshine-client_0.2.6_amd64.deb`。若状态来自 v1，先创建新授权码，再运行 `sudo sunshine-client pair replace --interactive`；Client 会归档旧账户文档，执行日志异常时则保留所有原件并明确报错。若安装器没有交互终端，稍后手动运行该命令。
...
```sh
sudo installer -pkg ./sunshine-client-0.2.6-macos-arm64-unsigned.pkg -target /
